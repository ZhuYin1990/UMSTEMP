...
public class LoginController extends BaseController {
		...
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			if(null == code || "".equals(code)){
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){
					String user_id = KEYDATA[0];
					String password  = KEYDATA[1];
					User parameterUser = new User();
					String passwd = new SimpleHash("SHA-1", user_id, password).toString();	//密码加密
					parameterUser.setUser_id(user_id);
					parameterUser.setPassword(passwd);
					User user = userService.getUserByNameAndPwd(parameterUser);
					if(user != null){
						parameterUser.setLast_login_time(DateUtil.getTime().toString());
						parameterUser.setLast_login_ip(this.getRemortIP());
						parameterUser.setId(user.getId());
						userService.updateLastLogin(parameterUser);
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu){
		...
		try{
			...
			if (user != null) {
				List<Menu> userMenuList = (List<Menu>)session.getAttribute(Const.SESSION_USERMENU);
				//if(userMenuList == null) {
					userMenuList = menuService.getUserMenu(user.getId());
					session.setAttribute(Const.SESSION_USERMENU, userMenuList);
				//}
