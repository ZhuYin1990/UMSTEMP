	...
	<div class="row-fluid">
			...
			<table>
				<tr>
					...
					<td><input class="span10 date-picker" name="lastLoginStart" id="lastLoginStart"  value="${pd.lastLoginStart}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:100px;" placeholder="最近登录开始" title="最近登录开始"/></td>
					<td><input class="span10 date-picker" name="lastLoginEnd" name="lastLoginEnd"  value="${pd.lastLoginEnd}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:100px;" placeholder="最近登录结束" title="最近登录结束"/></td>
						...
						<c:forEach items="${userList}" var="user" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									<c:if test="${user.USER_ID != 'admin'}"><label><input type='checkbox' name='ids' value="${user.USER_ID }" id="${user.EMAIL }" alt="${user.PHONE }"/><span class="lbl"></span></label></c:if>
									<c:if test="${user.USER_ID == 'admin'}"><label><input type='checkbox' disabled="disabled" /><span class="lbl"></span></label></c:if>
								</td>
								<td class='center' style="width: 30px;">${vs.index+1}</td>
								<td>${user.SERIAL_NUMBER }</td>
								<td><a>${user.USER_ID }</a></td>
								...
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<c:choose>
											...
											<c:otherwise>
												 <a class='btn btn-mini btn-danger' title="删除" onclick="delUser('${user.ID }','${user.USER_ID }');"><i class='icon-trash'></i></a>
											</c:otherwise>
										</c:choose>
									</div>
								</td>
