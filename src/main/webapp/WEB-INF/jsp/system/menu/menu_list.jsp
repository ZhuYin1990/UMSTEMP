...
<script type="text/javascript">
	...
	function openClose(parent_id,curObj,trIndex){
		var txt = $(curObj).text();
		if(txt=="展开"){
			$(curObj).text("折叠");
			$("#tr"+parent_id).after("<tr id='tempTr"+parent_id+"'><td colspan='5'>数据载入中</td></tr>");
			if(trIndex%2==0){
				$("#tempTr"+parent_id).addClass("main_table_even");
			}
			var url = "<%=basePath%>system/menu/subMenu.do?parent_id="+parent_id+"&guid="+new Date().getTime();
			$.get(url,function(data){
				if(data.length>0){
					var html = "";
					$.each(data,function(i){
						html = "<tr style='height:24px;line-height:24px;' name='subTr"+parent_id+"'>";
						html += "<td></td>";
						...
						html += "</tr>";
						$("#tempTr"+parent_id).before(html);
					});
					$("#tempTr"+parent_id).remove();
					if(trIndex%2==0){
						$("tr[name='subTr"+parent_id+"']").addClass("main_table_even");
					}
				}else{
					$("#tempTr"+parent_id+" > td").html("没有相关数据");
				}
			},"json");
		}else{
			$("#tempTr"+parent_id).remove();
			$("tr[name='subTr"+parent_id+"']").remove();
			$(curObj).text("展开");
		}
	}
</script>
